...
struct VTKWriter {
 ...
 public:
  ...
  VTKWriter(const std::string& name,const std::string& path,bool binary,const BBox& bb,const Eigen::Matrix<int,3,1>& nrCell,bool center)
    :_os(path.c_str(),binary ? std::ios_base::binary : std::ios_base::out),
     _points(binary ? std::ios_base::binary : std::ios_base::out),
     _cells(binary ? std::ios_base::binary : std::ios_base::out),
     _cellTypes(binary ? std::ios_base::binary : std::ios_base::out),
     ...
     _binary(binary) {
    Vec3T ext=(bb.maxCorner()-bb.minCorner()).template cast<T>();
    ...
    if(center) {
      Vec3T origin=bb.minCorner().template cast<T>()+spacing*0.5f;
      ...
      Vec3T origin=bb.minCorner().template cast<T>();
