...
void readJoints(std::vector<Joint<T>>& joints, int parentId, const tinyxml2::XMLElement* g) {
  ...
  //read trans
  {
    ...
    joint._q=PHYSICSMOTION::parsePtreeDef<Vec4T>(*g,"<xmlattr>.quat","1 0 0 0");
  }
  //read geometry
  if(g->FirstChildElement("geom")->FindAttribute("type") == NULL) {
    //capsule
    const tinyxml2::XMLElement* gg=g->FirstChildElement("geom");
    Vec6T ft=PHYSICSMOTION::parsePtreeDef<Vec6T>(*gg,"<xmlattr>.fromto","0 0 0 0 0 0");
    joint._c._radius=PHYSICSMOTION::get<T>(*gg,"<xmlattr>.size");
    joint._c._len=Vec3T(ft[3]-ft[0], ft[4]-ft[1],ft[5]-ft[2]).norm();
    joint._c._x=Vec3T(ft[3]+ft[0], ft[4]+ft[1],ft[5]+ft[2])/2;
    joint._c._q=QuatT::FromTwoVectors(Vec3T::UnitX(),Vec3T(ft[3]-ft[0], ft[4]-ft[1],ft[5]-ft[2]).normalized());
...
void updateCapsule(std::vector<Joint<T>>& joints) {
  for(auto& j : joints) {
    j._c._x = j._x + j._q.toRotationMatrix()*j._c._x;
    j._c._q = (j._c._q * j._q).normalized();
    if(j._parent>=0) {
      const auto& p = joints[j._parent]._c;
      j._x = p._x + p._q.toRotationMatrix()*j._x;
      j._q = (j._q * p._q).normalized();
      j._c._x = p._x + p._q.toRotationMatrix()*j._c._x;
      j._c._q = (j._c._q * p._q).normalized();
    }
  }
}
